...
public class LinkedListExtraTests {
    ...
    private void checkList2() {
        //assertEquals(expected2.length, actual.size());
        LinkedListNode<Integer> current = actual.getHead();
        for (int i = 0; i < expected2.length; i++) {
            assertNotEquals(null, current);
            assertEquals(expected2[i], current.getData());
            current = current.getNext();
        }
        assertEquals(null, current);
        if (expected2.length == 0) {
            ...
            int length = expected2.length;
            assertEquals(expected2[length - 1], actual.getTail().getData());
        }
    }
    ...
    @Test(timeout = TIMEOUT)
    public void RecursiveRemoveDuplicate() {
        // [2, 2, 3, 4, 5, 3]
        createDuplicates();
        // [2, 3, 4, 5]
        actual.recursiveRemoveDuplicates(actual.getHead());
        expected2 = new Integer[4];
        expected2[0] = new Integer(2);
        expected2[1] = new Integer(3);
        expected2[2] = new Integer(4);
        expected2[3] = new Integer(5);
        checkList2();
    }
    ...
    public void RecursiveRemove() {
        // [0][1] -> [][1]
        createNoDuplicates(2);
        actual.recursiveRemove(actual.getHead(), 0);
        expected2 = new Integer[2];
        //expected2[0] = new Integer(null);
        expected2[1] = new Integer(1);
